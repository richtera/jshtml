	...
	target.prototype.onToken = function onToken(token, buffer) {
		...
		*/
		if(token)	{
			...
			this.context.content += buffer.substring(0, token.index);
		...
		*/
			...
			if(this.options.debug)	{
				for(var index = buffer.indexOf(newline); ~index && index < buffer.length; index = buffer.indexOf(newline, index + newline.length))	{
					this.line++;
				}
			}

			this.options.trace && console.log(this.line);
