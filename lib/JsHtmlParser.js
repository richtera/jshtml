...
function JsHtmlParser(writeCallback, options) {
	...
	function onParse(token) {
		switch (token.category) {
			...
			innerParser.pushContext(expressionSetDictionary.text, new StackableWriter(innerParser.getWriter(), htmlWriteOptions));
			break;
			...
			case 'tag':
			innerParser.getWriter().call(null, '{');
			innerParser.pushContext(expressionSetDictionary.tagHead, new StackableWriter(innerParser.getWriter(), htmlWriteOptions));
			innerParser.getWriter().write(token.match[0]);
			break;
			...
			innerParser.getWriter().write(token.match[0]);
			innerParser.popContext();
			innerParser.pushContext(expressionSetDictionary.tagBody, new StackableWriter(innerParser.getWriter(), htmlWriteOptions));
			break;
